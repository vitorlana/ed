...
#include "base.h"
    ...
    void countSort(planeta *a, int size, int k)
    {
        ...
        {
            c[k < a[j].n_planeta.size() ? (int)(unsigned char) a[j].n_planeta[k] + 1 : 0]++;	
           	//cout << c[j] << endl;
        }
    ...
    void mergeSort_mes(planeta *vetor, int posicaoInicio, int posicaoFim) 
    {
        if(posicaoInicio == posicaoFim) return;
        ...
        while(i < metadeTamanho + 1 || j  < posicaoFim + 1) {
            if (i == metadeTamanho + 1 ) {
                ...
                vetorTemp[k].tn_planeta = vetor[j].tn_planeta;
                vetorTemp[k].n_planeta = vetor[j].n_planeta;
                vetorTemp[k].num_mes = vetor[j].num_mes;
                j++;
                k++;
            }
            else {
                if (j == posicaoFim + 1) {
                    vetorTemp[k].tn_planeta = vetor[i].tn_planeta;
                    vetorTemp[k].n_planeta = vetor[i].n_planeta;
                    vetorTemp[k].num_mes = vetor[i].num_mes;
                    i++;
                    k++;
                }
                else {
                    if (vetor[i].num_mes <= vetor[j].num_mes) {
                        vetorTemp[k].tn_planeta = vetor[i].tn_planeta;
                        vetorTemp[k].n_planeta = vetor[i].n_planeta;
                        vetorTemp[k].num_mes = vetor[i].num_mes;
                        i++;
        ...
        for(i = posicaoInicio; i <= posicaoFim; i++) {
            ...
            vetor[i].num_mes = vetorTemp[i - posicaoInicio].num_mes;
        }
        delete[] vetorTemp;    
}   
# endif